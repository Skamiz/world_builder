...
local function use_selector(player, indicator)
	if player:get_player_control().aux1 then
		...
		local pos = vector.round(world_builder.pointed_pos(player, pointing_distance, "under"))
...
minetest.register_globalstep(function(dtime)
	for player, p_data in pairs(players) do
		if player:get_wielded_item():get_name() == modprefix .."selector" then
			local new_pos = vector.round(world_builder.pointed_pos(player, pointing_distance, "under"))

			if not p_data.selector.pos then
				update_selection(player)
			end
			...
			if ((pos_1 or pos_2) and not (pos_1 and pos_2)) then
				update_selection(player, pos_1 or pos_2, vector.round(world_builder.pointed_pos(player, pointing_distance, "under")))
			end

		elseif p_data.selector.pos then
